...
<script type="text/javascript">
	...
	function check(){
		// 设置表单中图标的裁切信息
			...
			$('#ids').val(ids);
			$('#form').form('submit',{
				url:'${appPath}/commonContent/update.action',
	...
	function approveAction(){
			...
			$('#ids').val(ids);
			$('#form').form('submit',{
				url:'${appPath}/commonContent/approveAction.action',
				success:function(result){
					var r = $.parseJSON(result);
					$.messager.show({
						title:'提示',
					...
					<form  id="form" method="post" enctype="multipart/form-data">
						...
						<table >
							<tr>
								<td class="tt">类别：</td>
								<td>
									<input  id="infoCateId" name="infoCateId" value="${vo.infoCateId}" class="easyui-combotree" data-options="url:'${appPath}/infoCate/allInfoTree.action',parentField:'parentId',lines:true,cascadeCheck:false,checkbox:true,multiple:true,onlyLeafCheck:true,required:true" style="width:170px;" />
								</td>
							</tr>
							<tr>
								<td class="tt">标题：</td>
							...
							<c:if test="${vo.isTop!=null && vo.isTop!=''}">
								<tr id="top">
								<td class="tt">是否头条：</td>
								<td>
									<select class="" name="isTop" id="isTop">
										<c:if test="${vo.isTop=='1' }">
											<option value="1" selected="selected">是</option><option value="0" >否</option>
										</c:if>
										<c:if test="${vo.isTop=='0' }">
											<option value="1">是</option><option value="0" selected="selected">否</option>
										</c:if>
									</select>
								</td>
							</tr>
