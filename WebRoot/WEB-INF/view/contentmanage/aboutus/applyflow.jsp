	...
	function check(){
		$('#form').form('submit',{
			...
			success:function(result){
				...
				if(r.status){
					setTimeout(function(){
						location.href='${appPath}/aboutus/applyflow.action?id=${menuid}';
					},1000);
				}
				...
				<div>
					...
					<input type="hidden" name="nmenuid" value="${menuid}"/>
						<table >
							<tr>
								<th>标题：</th>
								<td><input name="ctitle"  id="title"  value="${vo.ctitle }" style="width: 680px;" class="easyui-validatebox" data-options="required:true" />
							<tr>
								<th>入会流程：</th>
								<td>
