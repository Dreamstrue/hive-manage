...
<script type="text/javascript">
	...
	function check(menuid,backId){
		if(validateLawName()){
			$('#ids').val(ids);
			$('#form').form('submit',{
				url:'${appPath}/policy/update.action',
	...
	function approveAction(menuid,backId){
		if(validateLawName()){
			$('#ids').val(ids);
			$('#form').form('submit',{
				url:'${appPath}/policy/approveAction.action',
				success:function(result){
					var r = $.parseJSON(result);
					$.messager.show({
						title:'提示',
	...
	function errorAction(menuid,backId){
			var id = $('#nlawid').val();
			$('#form').form('submit',{
				url:'${appPath}/policy/errorAction.action?id='+id,
				success:function(result){
					var r = $.parseJSON(result);
					$.messager.show({
						title:'提示',
	...
	function backUrl(menuid,backId){
		setTimeout(function(){
			if(backId!='0'){
				location.href='${appPath}/policy/allmanage.action?id='+backId;
			}else {
...
<c:if test="${opType=='update' }">
	<div id="div1" >当前位置：政策法规&nbsp;&gt;&nbsp;修改</div>
</c:if>
<c:if test="${opType=='approve' }">
	<div id="div1" >当前位置：政策法规&nbsp;&gt;&nbsp;审核</div>
</c:if>
<c:if test="${backId!='0' }">
				...
				<div>
					<form  id="form" method="post" enctype="multipart/form-data">
						...
						<table >
							<tr>
								<td  class="tt">法规标题：</td>
								<td><input name="clawname"  id="clawname"  value="${vo.clawname}" style="width: 630px;"/></td>
							</tr>
							<tr>
								<td  class="tt">概要内容：</td>
								<td>
