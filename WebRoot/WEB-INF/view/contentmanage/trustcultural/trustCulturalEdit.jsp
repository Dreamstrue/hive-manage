...
<script type="text/javascript">
		...
		var ids = new Array;
	function check(){
		if(checkculturalValue()){
			$('#ids').val(ids);
			$('#form').form('submit',{
				url:'${pageContext.request.contextPath}/cultural/update.action',
				success:function(result){
					...
					setTimeout(function(){
						location.href='${pageContext.request.contextPath}/cultural/manage.action?id=${vo.nmenuid}&pid=${pid}';
					},1000);
				}
			});
		}
	...
	function approveAction(){
			$('#ids').val(ids);
			$('#form').form('submit',{
				url:'${pageContext.request.contextPath}/cultural/approveAction.action',
				success:function(result){
					var r = $.parseJSON(result);
					$.messager.show({
						title:'提示',
...
<div id="div1" >当前位置：诚信文化&nbsp;&gt;&nbsp;修改</div>
</c:if>
<c:if test="${opType=='approve' }">
<div id="div1" >当前位置：诚信文化&nbsp;&gt;&nbsp;审核</div>
</c:if>
					...
					<form  id="form" method="post" enctype="multipart/form-data">
						...
						<table >
							<tr>
								<td class="tt">栏目选择：</td>
								<td>
									<input id="nmenuid" name="nmenuid" class="easyui-combobox" data-options="url:'${pageContext.request.contextPath}/navMenu/subNavMenu.action?id=${pid}',valueField:'id',textField:'text',required:true " style="width:170px;" value="${vo.nmenuid}" />
								</td>
							</tr>
							<tr>
								<td class="tt">标题：</td>
							...
							<tr>
								<td class="tt">所属来源：</td>
								<td><input name=cfrom  id="cfrom"  value="${vo.cfrom}" style="width: 630px;"/></td>
							</tr>
							<tr>
								<td class="tt">概要内容：</td>
								<td>
