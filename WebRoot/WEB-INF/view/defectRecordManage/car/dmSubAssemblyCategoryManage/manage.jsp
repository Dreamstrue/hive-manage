...
<script type="text/javascript">
	$(function() {
		$('#category_manage_datagrid').datagrid({
			url : '${appPath}/defectCar/dmSubAssemblyDatagrid.action',
			fit : true,
			fitColumns : false,
			border : false,
			pagination : true,
	...
	function category_manage_add_fn() {
		$('<div/>').dialog({
			id : 'add_dialog',
			href : '${appPath}/defectCar/toDmSubAssemblyAdd.action',
			width : 350,
			height : 200,
			modal : true,
			title : '添加类别',
			buttons : [ {
				...
				handler : function() {
					...
					$('#category_manage_add_form').form('submit', {
						url : '${appPath}/defectCar/dmSubAssemblyInsert.action',
	...
	function category_manage_del_fn(id) {
		$.messager.confirm('确认', '您确定要删除当前选中的类别吗？', function(r) {
			if (r) {
				$.post("${pageContext.request.contextPath}/defectCar/dmSubAssemblyDelete.action",{ids : id},function(result){
					if (result.status) {
		...
		if (rows.length > 0) {
			$.messager.confirm('确认', '您确定要删除当前选中的这些类别吗？', function(r) {
				if (r) {
					...
					//return false;
					$.ajax({
						url : '${pageContext.request.contextPath}/defectCar/dmSubAssemblyDelete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
	...
	function category_manage_edit_fn(id) {
		...
		$('<div/>').dialog({
			id : 'update_dialog',
			href : '${pageContext.request.contextPath}/defectCar/toDmSubAssemblyEdit.action?id=' + id,
			width : 350,
			height : 200,
			modal : true,
			title : '修改类别',
			buttons : [ {
				...
				handler : function() {
					...
					$('#category_manage_edit_form').form('submit', {
						url : '${pageContext.request.contextPath}/defectCar/dmSubAssemblyUpdate.action',
						success : function(result) {
							var r = $.parseJSON(result);
							if (r.status) {
