...
<html>
  <head>
	...
	$(function(){
		$('#entityName').textbox('textbox').focus(function() {
			queryAllIndustryEntity('','entityName');
    	});
	})
	</script>
  </head>
            ...
            <table id="toolbar" style="width: 100%">
                <tbody>
                    <tr>
                        ...
                        <td>行业类别：</td>
                        <td>
                        	<input class="easyui-combotree" id="industryId" name="industryId" data-options="prompt:'请选择行业类别',
                        	parentField:'pid',
                        ...
                        <td>评价对象：</td>
                        <td>
                        <input class="easyui-textbox" id="entityName" name="entityName" data-options="prompt:'点击选择评论对象'" style="width: 150px;" />
<!-- 						<input id="entityName" name="entityName" onclick="queryAllIndustryEntity('','entityName')" style="width: 150px;" /> -->
<!--                         	<input class="easyui-combobox" id="entityName" name="entityName" data-options="prompt:'请选择评价对象', -->
<!--                         		editable:false, -->
...
// 			url : '${pageContext.request.contextPath}/evaluationManage/datagrid.action',
	...
	function vote_manage_search_fn() {
		...
		if($("#endTime").datebox('getValue')==''
			&&$("#beginTime").datebox('getValue')==''
			&&$("#idCard").textbox('getValue')==''
			&&$("#mobilePhone").textbox('getValue')==''
			&&$("#voteUserName").textbox('getValue')==''
			&&$("#entityName").val()==''
			&&$("#industryId").combotree('getValues')==''
			&&$("#surveyName").combobox('getValue')==''){
			$.messager.confirm('提示','您确定要查询全部吗?，查询全部时间比较长，请耐心等待...',function(r){
			    if (r){
					$('#vote_questionnaire_datagrid').datagrid({url:'${pageContext.request.contextPath}/evaluationManage/datagrid.action',queryParams:serializeObject($('#searchForm'))});
			    }
			});
		}else{
// 			alert("结束时间："+$("#endTime").datebox('getValue'));
// 			alert("开始时间："+$("#beginTime").datebox('getValue'));
// 			alert("身份证号："+$("#idCard").textbox('getValue'));
// 			alert("电话号码："+$("#mobilePhone").textbox('getValue'));
// 			alert("参与人："+$("#voteUserName").textbox('getValue'));
// 			alert("实体名称："+$("#entityName").val());
// 			alert("行业id："+$("#industryId").combotree('getValues'));
// 			alert("问卷名称："+$("#surveyName").combobox('getValue'));
