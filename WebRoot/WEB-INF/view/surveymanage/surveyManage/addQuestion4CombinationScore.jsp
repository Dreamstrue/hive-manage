	...
	<body>
			...
			<form id="survey_question_add_form" method="post" style="width: 100%">
				...
				<table id="t1" class="tableForm" style="width: 100%;font-size: 12px;">
					...
					<tr>
						<th>必填</th>
						<td colspan="3">
							<input type="checkbox" checked="checked" id="ck_0" />
						</td>
						<td></td>
					</tr>
					...
					<tr>
						<td  colspan="5" style="background-color: #F4F4F4;text-align: left;width:100%;"><span style="font-size:14px;font-weight:bold;">添加问题↓ </span>&nbsp;&nbsp;<input type="button" class="easyui-linkbutton" value="增加问题" onclick="addOption1()"></td>
						<!-- 
					...
					<tr>
						<th>问题内容</th>
						<td colspan="3">
							<input name="optionText1" style="width:300px;" class="easyui-validatebox" data-options="required:true,validType:'maxLength[100]'"/>
						</td>
				...
				<table id="t2" class="tableForm" style="width: 100%;font-size: 12px;">
					<tr>
						<td  colspan="5" style="background-color: #F4F4F4;text-align: left;width:100%;"><span style="font-size:14px;font-weight:bold;">添加选项↓ </span>&nbsp;&nbsp;<input type="button" class="easyui-linkbutton" value="增加选项" onclick="addOption2()"></td>
						<!-- 
						<td colspan="3" style="background-color: #F4F4F4;text-align: right;width:50%;"><a id="saveSurveyBtn" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-ok'" >保存问卷</a></td>
						 -->
					</tr>
					<tr>
						<th>选项内容</th>
						<td><input name="optionText2" id="optionText_1" class="easyui-validatebox" data-options="required:true, validType:'maxLength[100]'" style="width: 300px;"/>&nbsp;&nbsp;<b>分值 </b><input name="score" class="easyui-numberspinner" data-options="min:0,max:1000,editable:true,required:true,missingMessage:'请填写选项分值'" value="0" style="width: 40px;" /></td>
						<td><span style="color:red;margin-right: -18px;"></span></td><!-- 在这儿调节文字对齐 -->
					</tr>
					<tr>
						<th>选项内容</th>
						<td><input name="optionText2" id="optionText_2" class="easyui-validatebox" data-options="required:true, validType:'maxLength[100]'" style="width: 300px;"/>&nbsp;&nbsp;<b>分值 </b><input name="score" class="easyui-numberspinner" data-options="min:0,max:1000,editable:true,required:true,missingMessage:'请填写选项分值'" value="0" style="width: 40px;" /></td>
						<td><span style="color:red;margin-right: -18px;"></span></td><!-- 在这儿调节文字对齐 -->
					</tr>
				</table>
			</form>
		...
		<script type="text/javascript">
			...
			function addOption1(){ 
	            ...
	            var td = $('<td colspan="4"><input name="optionText1" id="optionText_' + ck_numMax + '" class="easyui-validatebox" data-options="validType:\'maxLength[100]\'" style="width: 300px;"/>&nbsp;&nbsp;</td>'); //创建td对象
			...
			function addOption2(){ 
	            ...
	            var td = $('<td colspan="5"><input name="optionText2" id="optionText_' + ck_numMax + '" onblur="clearCheckBox(this.id)" class="easyui-validatebox" data-options="validType:\'maxLength[100]\'" style="width: 300px;"/>&nbsp;&nbsp;<b>分值 </b><input name="score" class="easyui-numberspinner" data-options="min:0,max:1000,editable:true,required:true,missingMessage:\'请填写选项分值\'" value="0" style="width: 40px;" />&nbsp;&nbsp;</td>'); //创建td对象
	            td.append(btndel); //将按钮添加到td中
	            var tr = $("<tr><th>选项内容</th></tr>");  //创建tr
	            tr.append(td);
	            $("#t2").append(tr);
	            $.parser.parse('#t2'); // easyui 在页面加载完成之后就完成了解析，后面我们通过 javascript 又动态添加的使用了 easyui 属性的元素就不会解析了，需要我们手动调用一下来完成解析
			} 
