			...
			<form id="editQuestion4Score_form" method="post" style="width: 99%">
				...
				<table class="tableForm" style="width: 100%;font-size: 12px;">
					...
					<tr>
						...
						<c:if test="${status.index > 1}">
						<td><input name="optionText" value="${item.optionText}" id="optionText_${status.index + 1}" class="easyui-validatebox" data-options="validType:'maxLength[100]'" style="width: 300px;"/>&nbsp;&nbsp;<b>分值 </b><input name="score" value="${item.score}" class="easyui-numberspinner" data-options="min:0,max:1000,editable:true,required:true,missingMessage:'请填写选项分值'" value="0" style="width: 40px;" />&nbsp;&nbsp;<b>可填空 </b><input type="checkbox" <c:if test='${item.requireinput == "1"}'>checked="checked"</c:if> id="ck_${status.index + 1}" />&nbsp;&nbsp;<input type='button' name="deleteBtn" title='删除' value='X'/></td>
						</c:if>
					</tr>
					</c:forEach>
				</table>
