...
<script type="text/javascript">
		$(function() {
			...
			var intendStatus = ${vo.intendStatus};
			$('#intendStatus1').val(intendStatus);
		});
...
<div id="div1" >当前位置：兑换管理&nbsp;&gt;&nbsp;订单明细</div>
				...
				<div>
					<form  id="form" method="post" enctype="multipart/form-data">
						...
						<table >
							...
							<c:if test="${vo.intendStatus=='1'}">
								<tr>
									<td class="tt">订单状态：</td>
									<td>
										<select id="intendStatus" name="intendStatus" style="width: 150px;"  disabled="disabled">
											<option value="1" selected="selected">1-已申请</option>
										</select>
									</td>
								</tr>
							</c:if>
							<c:if test="${vo.intendStatus=='2' || vo.intendStatus=='3' || vo.intendStatus=='4' || vo.intendStatus=='5'}">
								<tr>
									<td class="tt">订单状态：</td>
									<td>
										<select id="intendStatus1" name="intendStatus" style="width: 150px;" disabled="disabled">
								...
								<tr>
									<td class="tt">审核意见：</td>
									<td>
										<textarea name="auditOpinion" id="auditOpinion"  rows="8" cols="60" style="width:400px;height:50px;" disabled="disabled">${vo.auditOpinion}</textarea>
									</td>
								</tr>
							</c:if>
							<c:if test="${vo.intendStatus=='4' || vo.intendStatus=='5'}">
								<tr>
									<td class="tt">发货时间：</td>
									<td>
										<input name="sendTime"  id="sendTime"  value='<fmt:formatDate value="${vo.sendTime}" pattern="yyyy-MM-dd HH:mm:ss" />'  class="read" style="width: 200px;"/>
										
									</td>
								</tr>
							</c:if>
							<c:if test="${vo.intendStatus=='5'}">
								<tr>
									<td class="tt">确认收货时间：</td>
									<td>
										<input name="receiveTime"  id="receiveTime"  value='<fmt:formatDate value="${vo.receiveTime}" pattern="yyyy-MM-dd HH:mm:ss" />'  class="read" style="width: 200px;"/>
