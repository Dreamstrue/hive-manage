...
<html>
  ...
  <body>
  	<div class="easyui-layout" fit="true">
		...
		<div region="north" split="true" title="查询条件" border="false" class="p-search" style="overflow: hidden; height:80px; padding-left: 10px; padding-top: 5px;">
			<form action="javascript:void(0);" id="searchForm" style="width: 100%">
				...
				<label style="font-size: 12px;">证书名称:</label>
				<input name="certificateName" style="width: 215px;" />
	...
	<!-- 弹出窗口：添加数据 --> 
	<div id="mydialog" style="display:none;padding:5px;width:400px;height:300px;" title="资质信息审核"> 
	...
	<script type="text/javascript">
		...
		$(function() {
			$('#'+gridName).datagrid({
					url : '${appPath}/membermanage/listUncheckQualification.action',
					fit : true,
					fitColumns : true,
					border : false,
					pagination : true,
					...
					,{
	   					...
	   					width:20,
	   					formatter:function(value,row,index){
	   						return formatString('<img onclick="check(1,\'{0}\');" title="查看详情并审批" src="{1}"/>', row.nquaid, '${pageContext.request.contextPath}/resources/images/extjs_icons/book_open.png');
		...
		*/
		...
		function checkAdd(){
			if($("#form1").form('validate')){
				...
				$.post('${appPath}/membermanage/checkQualification.action',param, function (re) {
			        if (re.status) {
