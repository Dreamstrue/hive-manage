...
<script type="text/javascript">
	...
	function check(){
		...
		$('#form').form('submit',{
			url:'${appPath}/prizeInfo/update.action',
		...
		//删除附件的公共方法
	function delAnnex(){
		$.messager.confirm('提示','您确定要删除该图片?',function(b){
			if(b){
				$('#image').hide();
				$('#picturePath').show();
				$('#imageExist').val('no');
			...
			<td>
				<div>
					<form  id="form" method="post" enctype="multipart/form-data">
						...
						<input type="hidden" id="excNum" name="excNum" value="${vo.excNum}" />
						<input type="hidden" id="pictureName" name="pictureName" value="${vo.pictureName}" />
						<!-- 原新闻图标路径 -->
						...
						<table >
							<tr>
								<td class="tt">奖品名称：</td>
								<td><input name="prizeName"  id="prizeName" value="${vo.prizeName}" class="easyui-validatebox" data-options="required:true" style="width: 680px;"/></td>
							</tr>
							<tr>
								<td class="tt">奖品类别：</td>
								<td>
									<input id="prizeCateId" name="prizeCateId" value="${vo.prizeCateId}" class="easyui-combotree" data-options="url:'${appPath}/prizeCate/allPrizeTree.action',valueField:'id',textField:'infoCateName',required:true " style="width:200px;" />
								</td>
							</tr>
								<tr>
								<td class="tt">提供方：</td>
								<td>
								    <input id="entityCategory"  name="entityCategory"  class="easyui-combotree" value="${vo.entityCategory}" data-options="url:'${appPath}/entityCategoryManage/treegrid.action',parentField:'pid',lines:true,required:true "   style="width:200px;"/>
								</td>
							</tr>
							<tr>
								<td class="tt">奖品图片：</td>
									...
									<div id="preview">
								    	<img id="avatar" style="width: 120px; height: 120px;"
								    		<c:choose>
								    			...
								    			<c:otherwise> src="${appPath}/${vo.picturePath}"</c:otherwise>  
								    		</c:choose>
							...
							<tr>
								<td class="tt">奖品数量：</td>
								<td><input name="prizeNum"  id="prizeNum" value="${vo.prizeNum}" class="easyui-validatebox" data-options="required:true" style="width: 200px;" readonly="readonly"/></td>
							</tr>
							<tr>
								<td class="tt">兑换积分：</td>
								<td><input name="excIntegral"  id="excIntegral" value="${vo.excIntegral}" class="easyui-validatebox" data-options="required:true" style="width: 200px;"/></td>
							</tr>
							<tr>
								<td class="tt">奖品有效期：</td>
								<td>
									<input type='text' name="validDate" id='validDate' value="${vo.validDate}" style='width:200px'  class='easyui-validatebox' data-options="required:true" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"  />
								</td>
							</tr>
							<tr>
								<td class="tt">奖品说明：</td>
								<td>
									<textarea name="prizeExplain" id="prizeExplain"  rows="8" cols="60" style="width:680px;height:240px;visibility:hidden;">${vo.prizeExplain}</textarea>
									<script type="text/javascript">
